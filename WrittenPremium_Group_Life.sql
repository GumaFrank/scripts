...
Date 28-MAY-2024
Desc CHECKING FOR Written Premium


 */
Select A.V_NAME, A.* from
(
SELECT DISTINCT
       V_company_code,
...
SELECT DISTINCT
 ...
 WHERE     A.V_quotation_id = B.V_quotation_id(+)
       ...
       /*
       AND (   (D_bill_raised)
        BETWEEN :P_FM_DT AND :P_TO_DT
            OR 
            ( :P_FM_DT IS NULL AND :P_TO_DT IS NULL))
            
            )
            */
            ) A
            
            where A.V_POLICY_NO ='UG202100793355'
            AND TO_CHAR(A.D_BILL_RAISED, 'MON-YY') = 'APR-24'