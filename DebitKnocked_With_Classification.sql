...
Date 04-SEP-2024
Description  DebitKoncked With Classification 
the Key Changes 
V_PLAN_CODE: Added B.V_PLAN_CODE to the SELECT clause, fetched from the GNMT_POLICY table.
Classification: Introduced a DECODE function to classify V_PLAN_CODE values as either "Investment product" or "Traditional product."
GROUP BY: Included B.V_PLAN_CODE in the GROUP BY clause to accommodate the new column in the query.
...
SELECT V_POLICY_NO,
       ...
       V_PLAN_CODE,  -- Correctly added V_PLAN_CODE column
       DECODE(V_PLAN_CODE,  -- Updated DECODE function for Classification
  ...
  FROM (SELECT B.V_POLICY_NO,
               B.V_PLAN_CODE,  -- Ensure V_PLAN_CODE is selected from GNMT_POLICY table
...
GROUP BY V_POLICY_NO,
         ...
         NATIONAL_SALES_MANAGER,
         V_LOB_DESC,
         V_PLAN_CODE;  -- Added V_PLAN_CODE in the GROUP BY clause
/