/* Formatted on 17/02/2021 15:52:49 (QP5 v5.256.13226.35538) */
--WITH ParamDates AS (SELECT '01/05/2020' AS AsAt FROM DUAL)
SELECT * FROM 

(
SELECT u.V_COMPANY_CODE,
       u.V_COMPANY_BRANCH,
       REPLACE (V_COMPANY_NAME, ',', '') V_SCHEME,
       u.V_POLICY_NO,
       --       N_TOT_DR - NVL (N_RECEIPT_AMT, 0) N_OS_BAL_AMT,
       --       N_TOT_DR,
       --       NVL (N_RECEIPT_AMT, 0) N_RECEIPT_AMT,
       --       DECODE ( (N_TOT_DR - NVL (N_RECEIPT_AMT, 0)) - N_BILL_BAL_AMT,
       --               0, 0,
       --               (N_TOT_DR - NVL (N_RECEIPT_AMT, 0)) - N_BILL_BAL_AMT)
       --          N_UNALLOCATED,
       (SELECT NVL (SUM (N_TRN_AMT), 0)
          FROM gndt_bill_trans BT
         WHERE BT.v_policy_no = U.v_policy_no AND BT.V_BILL_BAL_TYPE = 'D')
          N_OS_BAL_AMT,
       (SELECT SUM (N_TRN_AMT)
          FROM gndt_bill_trans BT
         WHERE     BT.v_policy_no = U.v_policy_no
               AND BT.N_BILL_TRN_NO = 1
               AND BT.V_BILL_TYPE = 'DN')
          N_TOT_DR,
       (SELECT NVL(N_AMOUNT,0)
          FROM PPMT_OVERSHORT_PAYMENT PT
         WHERE PT.v_policy_no = U.V_POLICY_NO AND ROWNUM = 1)
          N_UNALLOCATED,
       ...
       (SELECT V_AGENT_CODE
         ...
         WHERE     p.N_AGENT_NO = q.N_AGENT_NO
               ...
               AND p.V_STATUS = 'A'
               AND NVL (q.V_COMPANY_CODE, 'X') = NVL (r.V_COMPANY_CODE, 'X')
               AND NVL (q.V_COMPANY_BRANCH, 'X') =
                      NVL (r.V_COMPANY_BRANCH, 'X')
               AND V_POLICY_NO = u.V_POLICY_NO)
          V_BROKER_CODE,
       (SELECT REPLACE (V_COMPANY_NAME, ',', '')
          FROM AMMT_POL_AG_COMM p, AMMM_AGENT_MASTER q, GNMM_COMPANY_MASTER r
         WHERE     p.N_AGENT_NO = q.N_AGENT_NO
               AND p.V_ROLE_CODE = 'SELLING'
               AND p.V_STATUS = 'A'
  ...
  FROM (  SELECT V_COMPANY_CODE,
            ...
            FROM (SELECT V_COMPANY_CODE,
                    ...
                    FROM (SELECT V_COMPANY_CODE,
                                 ...
                                 DECODE (
                                    ...
                                    'CN', - (N_TRN_AMT - NVL (N_SETTLED, 0)))
                                    N_BAL_AMT,
                                 TRUNC (SYSDATE) - TRUNC (D_TRN_DATE) DAYS,
                                 CASE
                                    WHEN TRUNC (SYSDATE) - TRUNC (D_TRN_DATE) <=
                                            30
                                    THEN
                                       30
                                    WHEN TRUNC (SYSDATE) - TRUNC (D_TRN_DATE) <=
                            ...
                            FROM GNDT_BILL_TRANS a,
                                 (  SELECT V_POLICY_NO,
                                     ...
                                     WHERE     V_POLICY_NO LIKE 'UG%'
                                           AND N_BILL_TRN_NO > 1
                                           --AND D_TRN_DATE <= '31-OCT-2012'
                                           AND NVL (D_TRN_DATE, TRUNC (SYSDATE)) <=
                                                  NVL (
                                                     :P_TO_DT,
                                                     TO_DATE (SYSDATE,
                                                              'DD/MM/YYYY'))
                                  --AND V_BILL_NO = 'DN171605'
                           ...
                           WHERE     a.V_POLICY_NO = b.V_POLICY_NO(+)
                                 ...
                                 AND a.V_POLICY_NO IN (SELECT V_POLICY_NO
                                                         FROM GNMT_POLICY
                                                        WHERE V_CNTR_STAT_CODE =
                                                                 'NB010')
                                 --AND D_AGE_START_DATE <= '31-OCT-2012'
                                 AND NVL (D_AGE_START_DATE, TRUNC (SYSDATE)) <=
                                        NVL ( :P_TO_DT,
       ...
       (  SELECT V_POLICY_NO, SUM (N_RECEIPT_AMT) N_RECEIPT_AMT
           ...
           WHERE     V_RECEIPT_TABLE = 'DETAIL'
                 ...
                 AND NVL (D_RECEIPT_DATE, TRUNC (SYSDATE)) <=
                        NVL ( :P_TO_DT, TO_DATE (SYSDATE, 'DD/MM/YYYY'))
        GROUP BY V_POLICY_NO) v,
       GNMM_COMPANY_MASTER w
 WHERE     u.V_POLICY_NO = v.V_POLICY_NO(+)
       AND NVL (u.V_COMPANY_CODE, 'X') = NVL (w.V_COMPANY_CODE(+), 'X')
       AND NVL (u.V_COMPANY_BRANCH, 'X') = NVL (w.V_COMPANY_BRANCH(+), 'X')
       
       )
       WHERE V_SCHEME LIKE '%NOKIA%'